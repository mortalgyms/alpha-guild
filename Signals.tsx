...
export default function Markets() {
  ...
  const { quotes, loading, streamStatus } = useLiveQuotes(symbols, 15000);
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <SectionHeader
        ...
        subtitle={`Stocks, crypto, and forex with freshness validation. Stream: ${streamStatus}.`}
        right={
        ...
        <div className="col-span-12 lg:col-span-9 rounded-2xl border border-border bg-card/40 backdrop-blur-xl p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              {q?.meta && (
                <Badge className={cn("border text-xs", q.meta.status === "live" ? "bg-success/15 text-success border-success/30" : q.meta.status === "delayed" ? "bg-warning/15 text-warning border-warning/30" : "bg-destructive/15 text-destructive border-destructive/30")}>
                  {q.meta.status} · {q.meta.provider}
                </Badge>
              )}
          ...
          <div className="mt-2 grid grid-cols-1 md:grid-cols-4 gap-2">
            {[
              { l: "Source Time", v: formatMarketTime(q?.meta?.sourceTimestamp) },
              { l: "Received", v: formatMarketTime(q?.meta?.receivedAt) },
              { l: "Local Time", v: formatMarketTime(q?.meta?.localTimestamp) },
              { l: "Delay", v: formatMarketDelay(q?.meta?.delayMs) },
            ].map((s) => (
              <div key={s.l} className="rounded-md border border-border bg-background/40 px-3 py-2 flex items-center justify-between">
                <span className="text-[10px] uppercase tracking-widest text-muted-foreground">{s.l}</span>
                <span className="font-mono text-xs font-semibold tabular-nums">{s.v}</span>
              </div>
            ))}
          </div>
        </div>
...
function formatMarketTime(value?: number | null) {
  if (!value) return "unknown";
  return new Date(value).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" });
}

function formatMarketDelay(value?: number | null) {
